...
def verify_user(
    ...
    if static_token and token == static_token:
        workspace_id = x_aegisure_workspace or os.getenv("AEGISURE_WORKSPACE_ID") or "local"
    ...
    if not secret:
        if os.getenv("AEGISURE_DEV_AUTH", "false").lower() == "true":
            return {"user_id": "dev-user", "email": "dev@aegisure.dev", "workspace_id": x_aegisure_workspace or "local", "auth_mode": "dev"}
    ...
    workspace_id = x_aegisure_workspace or claims.get("workspace_id") or f"ws_{user_id}"
