  ...
  return (
    <AppShell current="memory">
      <PageHeader eyebrow="Cross-agent memory" title="One Constitution, every agent aligned." action={<Button variant="secondary"><Download className="h-4 w-4" /> Export all</Button>}>
        AEGIS.md is the canonical source. Aegisure exports idempotent rule files for real coding agents without rewriting your project logic.
      </PageHeader>
      <div className="grid gap-4 p-4 md:grid-cols-2 md:p-6 lg:grid-cols-3 lg:p-10">
        {agentExports.map((target) => (
          <Card key={target}>
            ...
            <div className="mt-5"><Badge tone={target === "AEGIS.md" ? "accent" : "neutral"}>{target === "AEGIS.md" ? "Canonical" : "Export"}</Badge></div>
            <h2 className="mt-4 font-medium">{target}</h2>
            <p className="mt-2 text-sm leading-6 text-muted-foreground">
              {target === "AEGIS.md" ? "Canonical Aegisure Constitution." : "Generated from AEGIS.md so external agents receive the same rules."}
            </p>
          </Card>
        ))}
      </div>
